...
def astar(start, target, roads=None):
    ...
    opened_dict = {start: Node(index=start, parent_node=None, g_cost=0,
                               h_cost=huristic(lat1=roads[start.index].lat, lon1=roads[start.index].lon,
                                               lat2=roads[target.index].lat, lon2=roads[target.index].lon))}
    ...
    while sorted_opened_dict:
        ...
        if roads[selected_node_object.index].links is not None:
            ...
            for link in roads[selected_node_object.index].links:
                ...
                h_cost = huristic(lat1=roads[selected_node_object.index].lat, lon1=roads[selected_node_object.index].lon,
